...
public class GetTaskTest {
    ...
    Repository<Task, Void> emptyRepo = new TestTaskRepository();
    Repository<Task, Void> notEmptyReposiroy = new TestTaskRepository();
    ...
    public void givenRepositoryIsEmpty__whenRequestingATask__then404() throws Exception {
        TaskGetResponse actual = new GetTask(account -> emptyRepo).apply(TaskGetRequest.builder().account("account").taskId("task-id").build());

        actual.opt().status404().orElseThrow(() -> new AssertionError("expecting 404 got " + actual));
    }

    @Test
    public void givenRepositoryIsNotEmpty__whenRequestingExistingId__then200() throws Exception {
        TaskGetResponse actual = new GetTask(account -> notEmptyReposiroy).apply(TaskGetRequest.builder().account("account").taskId("existing-task").build());
    ...
    public void givenRepositoryIsNotEmpty__whenRequestingNotExistingId__then404() throws Exception {
        TaskGetResponse actual = new GetTask(account -> notEmptyReposiroy).apply(TaskGetRequest.builder().account("account").taskId("not-existing-task").build());
